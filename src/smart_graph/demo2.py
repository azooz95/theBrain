import gradio as gr
import os
from graph.main_graph2 import app
...
def chat_logic(user_input, uploaded_file):
    ...
    if effective_file:
        ...
        session["file_path_for_analysis"] = effective_file.name
    ...
    try:
        result = app.invoke({
            ...
            "file_path_for_analysis": session.get("file_path_for_analysis")
        })
        ...
        if ai_response:
            print("🤖 Agent response:", ai_response)  # Debug print
            session["chat"].append((ai_response, "bot"))
        else:
            print("⚠️ No AI response generated.")

        session["file_path_for_analysis"] = None  # Clear after use

    except Exception as e:
        print("❌ LangGraph exception:", e)
...
with gr.Blocks() as demo:
    gr.Markdown("## 🤖 Smart Multi-Agent Assistant — Contract, Document & Meeting")

    chatbot = gr.Chatbot(label="Conversation", height=600)
    msg = gr.Textbox(label="Message")
    file_upload = gr.File(label="Upload DOCX or Document", file_types=[".docx", ".pdf", ".png", ".jpg", ".jpeg", ".bmp", ".tiff"])
